...
export async function main(ns)
{
    ...
    while (true) {
        ...
        gangMemberNames.sort(sortBy(ns, SORT_TYPES.STR, DIRECTIONS.DESC)).sort(sortBy(ns, SORT_TYPES.STR_MULT, DIRECTIONS.DESC)).
        ...
        if (buyEquipment) {
            ...
            gangMemberNames.forEach((gangMemberName) =>
            {
                ...
                {
                    if (gangMemberInfo.upgrades.includes(equipment.name)) {
                        return
                    }
            ...
            if (hasAllEq) {
                ...
                gangMemberNames.forEach((gangMemberName) =>
                {
                    ...
                    augumentationssToBuy.filter((aug) => !aug.hack).forEach((equipment) =>
                    {
                        if (gangMemberInfo.augmentations.includes(equipment.name)) {
                            return
                        }
        ...
        if (gangMemberNames.length && new Date().getTime() > nextAscensionAttempt) {
            if (doAscension) {
                ...
                gangMembersToAscend.sort(sortBy(ns, SORT_TYPES.REPUTATION, DIRECTIONS.ASC)).slice(0, maxMembersToAscend).forEach((gangMemberName) =>
                {
